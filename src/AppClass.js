...
export default class AppClass extends Component {
  ...
  play = (userChoice) => {
    ...
    this.setState({
      ...
      result1: this.judgement1(computerChoice, choice[userChoice]),
      gameTurn: i = i + 1,
      winTurn: this.judgenum(this.judgement(choice[userChoice], computerChoice)),
      tieTurn: this.judgenum1(this.judgement(choice[userChoice], computerChoice)),
      loseTurn: this.judgenum2(this.judgement(choice[userChoice], computerChoice))
    })
  }
    ...
    return (
      <div>
        <div className='main'>
          ...
          <BoxClass title='Computer' item={this.state.comSelect} result={this.state.result1} />
        </div>
        ...
        <div className='text'>
          <div>게임 횟수: {this.state.gameTurn}</div>
          <div>이긴 횟수: {this.state.winTurn}</div>
          <div>비긴 횟수: {this.state.tieTurn}</div>
          <div>진 횟수: {this.state.loseTurn}</div>
        </div>
      </div>
    )
  }
